...
export const fetchmedia = async (req, res) => {
  ...
  if (!media || media.length===0) {
    ...
    return res.status(StatusCodes.OK).json({media:[]});
  }
       ...
       populate: {
         ...
         populate: {
           ...
           populate: [
             ...
             {
               path: 'topics', // Populate topics in media
               populate: {
                 path: 'Chapter', // Populate chapter in topics
                 populate: {
                   path: 'subject', // Populate subject in chapter
                   populate: {
                     path: 'standard', // Populate standard in subject
                   },
                 },
               },
             },
...
{
    const unverifiedMedia = await Media.find({ verified: false })
      .populate({
        path: 'topics',
        populate: {
          path: 'Chapter',
      ...
      if (!unverifiedMedia || unverifiedMedia.length === 0) {
        //throw new NotFoundError(`class not found ${standard}`);
        return res.status(StatusCodes.OK).json({ media: [] });
      }

      res.status(StatusCodes.OK).json({ media: unverifiedMedia });
}
};
...
export const pending = async (req,res)=>{
  const teacherId=req.user.userId;
    const media = await Media.find({ teacher: teacherId,verified:false })
      .populate({
        path: 'topics',
        populate: {
          path: 'Chapter',
